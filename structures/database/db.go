...
import (
	...
	ls "github.com/Khayrullinn66/L0Nats/structures/localStore"
	so "github.com/Khayrullinn66/L0Nats/structures/structOrder"
...
func InitDb(port string) (Database, error) {
	db, err := gorm.Open(postgres.Open(fmt.Sprintf("host=localhost port=%s user=khayrullin password=7712 dbname=postgres sslmode=disable", port)))
	if err != nil {
		return Database{}, err
	}
